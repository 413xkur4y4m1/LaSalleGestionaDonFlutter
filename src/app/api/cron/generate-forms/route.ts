...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const studentDoc of studentsSnapshot.docs) {
      ...
      // 1. --- BUSCAMOS PRÉSTAMOS EXPIRADOS HACE MÁS DE 24 HORAS ---
      ...
      console.log(
        `[CRON | g-forms]: Estudiante ${studentData.nombre || studentId} tiene ${loansSnapshot.size} préstamos listos para formulario.`
      );
      ...
      for (const loanDoc of loansSnapshot.docs) {
        ...
        const processPromise = async () => {
          // 2. --- VERIFICAMOS QUE NO EXISTA YA UN FORMULARIO ---
          const existingFormQuery = formsRef
          ...
          if (!existingFormSnap.empty) {
            console.log(
              ` -> Formulario para préstamo ${prestamoId} ya existe. Saltando.`
            );
            return;
          }
          
          console.log(
            ` -> Generando formulario para préstamo ${prestamoId} (${loanData.nombreMaterial}).`
          );
    ...
    console.log(
      `--- [CRON | generate-forms]: Finalizado. ${formsSentCount} formularios generados. ---\n`
    );
    
    return NextResponse.json({ 
      message: `Proceso completado. Se generaron ${formsSentCount} formularios de seguimiento.` 
    });
