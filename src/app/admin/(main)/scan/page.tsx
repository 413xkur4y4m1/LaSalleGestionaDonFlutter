...
import { LoaderCircle, CheckCircle, XCircle, AlertTriangle, ScanLine } from 'lucide-react';
...
// --- Componente Principal de la Página de Escaneo ---
const ScanPageContent = () => {
    ...
    const fetchPrestamoDetails = async (codigo: string) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.message || 'Error al buscar el préstamo.');
            }
            setPrestamo(data);
    ...
    const handleActivate = async () => {
        ...
        try {
            const response = await fetch('/api/prestamos/activate', {
                method: 'POST',
            ...
            const data = await response.json();
            if (!response.ok) {
                throw new Error(data.message || 'No se pudo activar el préstamo.');
            }
    ...
    const handleScanError = (error: unknown) => {
        if (error instanceof Error) {
            ...
            toast.error("Ocurrió un error de escaneo desconocido.");
        }
    }
