"use client";

import React, { useState, useEffect } from 'react';
import TopNavbar from '../organisms/TopNavbar';
import SidebarMenu from '../organisms/SidebarMenu';
import Chatbot from '../organisms/Chatbot';
import GrupoModal from '../organisms/GrupoModal';
import * as firestoreOperations from '@/lib/firestore-operations-client';
import { getAuth } from "firebase/auth";
import { useAuth } from '@/components/providers/NextAuthProvider';

interface DashboardTemplateProps {
  children: React.ReactNode;
}

const DashboardTemplate: React.FC<DashboardTemplateProps> = ({ children }) => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [needsGrupo, setNeedsGrupo] = useState(false);
  const { user } = useAuth();

  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        const auth = getAuth();
        const currentUser = auth.currentUser;

        if (!currentUser) return;

        const studentData = await firestoreOperations.getStudentData(currentUser.uid);
        if (studentData) {
          setNeedsGrupo(studentData.grupo === '');
        }
      } catch (error) {
        console.error("Error fetching student data:", error);
      }
    };

    fetchStudentData();
  }, [user]);

  const handleGrupoSubmit = async (grupo: string) => {
    try {
      const auth = getAuth();
      const currentUser = auth.currentUser;
      if (!currentUser) return;

      await firestoreOperations.updateStudentGrupo(currentUser.uid, grupo);
      setNeedsGrupo(false); // Actualizamos estado sin recargar la página
    } catch (error) {
      console.error("Error updating student group:", error);
    }
  };

  if (needsGrupo) {
    return <GrupoModal isOpen={needsGrupo} onSubmit={handleGrupoSubmit} />;
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <TopNavbar onMenuToggle={() => setIsSidebarOpen(!isSidebarOpen)} />

      <div className="flex relative">
        <SidebarMenu isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />

        {/* Overlay for mobile */}
        {isSidebarOpen && (
          <div
            className="md:hidden fixed inset-0 bg-black/50 z-40"
            onClick={() => setIsSidebarOpen(false)}
          />
        )}

        <main className="flex-1 p-4 sm:p-6 md:p-8 lg:p-10 ml-0 md:ml-[200px] lg:ml-[220px] xl:ml-[240px] min-h-[calc(100vh-4rem)]">
          {children}
        </main>
      </div>

      <Chatbot />
    </div>
  );
};

export default DashboardTemplate;
